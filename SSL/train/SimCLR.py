...
def NT_Xent(z, temperature, device): # z.size(): (2batch_size, out_dim)
...
def simclr(model, empty_model, train_loader, test_loader, pretrain_loader, optimizer, lr_scheduler, device, epoch_num, logdir):
    ...
    for epoch in range(epoch_num):
        ...
        for inputs, _ in pretrain_loader:
            ...
            optimizer.zero_grad()
            out1 = model(inputs1)  # (batch_size, out_dim)
            out2 = model(inputs2)  # (batch_size, out_dim)
            z = torch.cat([out1, out2]) # i, i + batch_size -> positive pair, (2*batch_size, out_dim)
    ...
    # linear_acc(model, epoch_num, out_dim, 10, train_loader, test_loader, device)
    writer.close()
